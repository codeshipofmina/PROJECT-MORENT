    ...
    try {
      ...
      if (newAvatar) {
        ...
        if (!allowedTypes.includes(newAvatar.type)) {
          setError("Only JPEG, PNG or GIF image files can be uploaded.");
          setLoading(false);
          return;
        }
        ...
        if (uploadError) {
          setError("Photo could not be loaded: " + uploadError.message);
          setLoading(false);
          return;
        }
      ...
      if (updateError) {
        setError("Profile could not be updated: " + updateError.message);
      } else {
